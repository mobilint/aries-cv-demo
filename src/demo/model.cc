...
Model::Model(const ModelSetting& model_setting, mobilint::Accelerator& acc,
    ...
    if (mPipeline->supportsAsync()) {
        mc.setAsyncPipelineEnabled(true);
    }

    mModel = mobilint::Model::create(model_setting.mxq_path, mc, sc);
    if (!sc || !mModel) {
        throw std::runtime_error("Failed to create model: " + model_setting.mxq_path +
                                 ", status=" + std::to_string(static_cast<int>(sc)));
    }
    mModel->launch(acc);

    if (model_setting.model_type == ModelType::OBJECT &&
        (model_setting.post_type == PostProcessType::YOLO11 ||
         model_setting.post_type == PostProcessType::YOLO26_INLINE ||
...
bool Model::supportsAsync() const { return mPipeline->supportsAsync(); }

mobilint::Future<float> Model::submitAsync(const cv::Mat& frame, cv::Size size, int worker_index,
                                           mobilint::StatusCode& sc) {
    ...
    return mPipeline->submitAsync(frame, mModelSetting, worker_context, *mModel, workspace, sc);
}

cv::Mat Model::completeAsync(mobilint::Future<float>& future, const cv::Mat& frame, cv::Size size,
                             int worker_index) {
    PipelineWorkspace& workspace = getWorkspace(worker_index);
    const WorkerContext worker_context{worker_index};

    mobilint::StatusCode sc;
    auto outputs = future.get(sc);
    workspace.npu_bench.end();
    if (!sc) {
        printf("[ERR] async get failed: worker=%d, error_code=%d\n", worker_index,
               static_cast<int>(sc));
        cv::Mat fallback;
    ...
    DetectionResult result =
        mPipeline->postprocess(outputs, frame, mModelSetting, worker_context, workspace);
    if (!result.boxes.empty()) {
        smoothDetectionScores(worker_index, result);
    } else if (result.display_scores.size() != result.scores.size()) {
...
cv::Mat Model::inference(const cv::Mat& frame, cv::Size size, int worker_index) {
    ...
    // NPU metric is intentionally scoped to qbruntime Model::infer() only.
    // Preprocess, postprocess, and render are measured by separate debug benchmarkers below.
