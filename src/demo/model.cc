...
cv::Mat Model::inference(const cv::Mat& frame, cv::Size size, int worker_index,
    ...
    auto pipeline_config = mModelSetting.pipeline_config;
    pipeline_config.draw_score_text = draw_score_text;
    const RenderContext render_context{size, pipeline_config};
