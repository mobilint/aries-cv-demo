...
public:
    ...
    mobilint::Future<float> submitAsync(const cv::Mat& frame, cv::Size size, int worker_index,
                                        mobilint::StatusCode& sc);
    cv::Mat completeAsync(mobilint::Future<float>& future, const cv::Mat& frame, cv::Size size,
                          int worker_index);
