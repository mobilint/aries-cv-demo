...
struct PipelineWorkspace {
    ...
    // Per-worker input/scratch buffers; workspace-scoped so an in-flight async
    // input stays valid while the lane preprocesses another worker's frame.
    cv::Mat ws_resized;
    cv::Mat ws_padded;
    cv::Mat ws_rgb;
    mobilint::NDArray<uint8_t> ws_u8;
    mobilint::NDArray<float> ws_f32;
    size_t ws_u8_size = 0;
    size_t ws_f32_size = 0;
...
struct DetectionResult {
    ...
    cv::Mat seg_label_mask;
    cv::Mat seg_score_mask;
};
...
public:
    ...
    // Launches inferAsync into workspace buffers without blocking; the caller
    // runs postprocess/render on the Future later. Default: sync only.
    virtual bool supportsAsync() const { return false; }

    virtual mobilint::Future<float> submitAsync(const cv::Mat& frame,
                                                const ModelSetting& model_setting,
                                                const WorkerContext& worker_context,
                                                mobilint::Model& model,
                                                PipelineWorkspace& workspace,
                                                mobilint::StatusCode& sc) {
        (void)frame; (void)model_setting; (void)worker_context; (void)model; (void)workspace;
        sc = mobilint::StatusCode::Model_NotSupported;
