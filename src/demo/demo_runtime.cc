...
bool isMla400Mode(const std::string& mode_name) { return mode_name == "mla400"; }
}  // namespace
...
void DemoRuntime::loadManifest(const std::string& mode) {
    ...
    if (isMla400Mode(mCurrentMode)) {
        mDrawScoreText = false;
        mPerformanceDisplayMode = PerformanceDisplayMode::AVG_FPS_ONLY;
    } else {
        mDrawScoreText = true;
        mPerformanceDisplayMode = PerformanceDisplayMode::TILE_FPS;
    }
}
...
float DemoRuntime::computeAverageNpuMs() const {
    float sum = 0.0f;
    int count = 0;
    for (size_t i = 0; i < mWorkerEnabledSize; ++i) {
        if (mWorkerEnabled[i].load(std::memory_order_relaxed) == 0) continue;
        if (i >= mManifest.layout.worker_tiles.size()) continue;
        if (!mWorkerLayoutValid.empty() && !mWorkerLayoutValid[i]) continue;

        const auto& worker = mManifest.layout.worker_tiles[i];
        if (worker.model_index < 0 || static_cast<size_t>(worker.model_index) >= mModels.size()) {
            continue;
        }

        const double npu_ms = mModels[worker.model_index]->getLastNpuMs(static_cast<int>(i));
        if (npu_ms <= 0.0) continue;
        sum += static_cast<float>(npu_ms);
        count++;
    }
    if (count == 0) return 0.0f;
    return sum / static_cast<float>(count);
...
bool DemoRuntime::keyHandler(int key) {
    ...
    if (key == 'd') {
        switch (mPerformanceDisplayMode) {
            case PerformanceDisplayMode::TILE_FPS:
                mPerformanceDisplayMode = PerformanceDisplayMode::AVG_FPS_ONLY;
                break;
            case PerformanceDisplayMode::AVG_FPS_ONLY:
                mPerformanceDisplayMode = PerformanceDisplayMode::OFF;
                break;
            case PerformanceDisplayMode::OFF:
                mPerformanceDisplayMode = PerformanceDisplayMode::TILE_FPS;
                break;
        }
    } else if (key == 't')
