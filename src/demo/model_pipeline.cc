...
std::string detectionLabelName(int label, const PipelineConfig& config) {
    if (label >= 0 && label < static_cast<int>(config.labels.size())) {
...
void renderDetections(const DetectionResult& result, const cv::Mat& frame,
    ...
    for (size_t i = 0; i < result.boxes.size(); ++i) {
        ...
        if (render_context.pipeline_config.draw_score_text) {
            ...
            std::string text;
            if (render_context.pipeline_config.draw_label_text) {
                text = detectionLabelName(label, render_context.pipeline_config) + " ";
            }
            char conf_text[32];
            std::snprintf(conf_text, sizeof(conf_text), "%.2f", score);
            text += conf_text;
            int text_y = std::max(14, y1 - 6);
            cv::putText(workspace.result_frame, text, cv::Point(x1, text_y),
                        cv::FONT_HERSHEY_SIMPLEX, 0.5, clr, 1, cv::LINE_AA);
        }
    }

    if (detected && render_context.pipeline_config.draw_detection_border) {
...
std::unique_ptr<ModelPipeline> createModelPipeline(const ModelSetting& model_setting) {
    if (model_setting.model_type == ModelType::OBJECT) {
        switch (model_setting.post_type) {
        case PostProcessType::YOLO11:
            return std::make_unique<Yolo11DetectionPipeline>();
        case PostProcessType::YOLO26_INLINE:
            return std::make_unique<Yolo26DetectionPipeline>();
        case PostProcessType::ANCHORLESS:
            return std::make_unique<YoloAnchorlessDetectionPipeline>();
        case PostProcessType::DFLFREE:
        case PostProcessType::NMSFREE:
            return createObjectDflFreePipeline();
        default:
            break;
        }
    }
    if (model_setting.model_type == ModelType::FACE &&
        model_setting.post_type == PostProcessType::ANCHORLESS) {
        return createFacePipeline();
    }
    if (model_setting.model_type == ModelType::POSE &&
        model_setting.post_type == PostProcessType::DFLFREE) {
        return createPosePipeline();
    }
    if (model_setting.model_type == ModelType::SEGMENTATION &&
        model_setting.post_type == PostProcessType::DFLFREE) {
        return createSegPipeline();
    }
    if (model_setting.model_type == ModelType::CLASSIFICATION &&
        model_setting.post_type == PostProcessType::CLASSIFICATION) {
        return createClassificationPipeline();
    }
    if (model_setting.model_type == ModelType::SSD) {
        return createSsdPipeline();
    }
    throw std::invalid_argument("Unsupported model/post type combination.");
}
