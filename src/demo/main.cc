...
void printUsage(const DemoRegistry& registry, const char* argv0) {
    std::cout << "Usage: " << argv0 << " [demo-id] [--debug] [--verbose] [--json-path path]\n";
    std::cout << "       " << argv0 << " --list\n\n";
    std::cout << "Options:\n";
    std::cout << "  --debug, -d          Print per-stage inference benchmarks to console and save tracing JSON\n";
    std::cout << "  --verbose, -v        Print detailed runtime/feed/video diagnostics\n";
    std::cout << "  --json-path <path>   Save tracing JSON to path when --debug is enabled (default: trace.json)\n\n";
...
LauncherKey readLauncherKey() {
    ...
    if (ch == '\r' || ch == '\n') return LauncherKey::Enter;
    if (ch == 'q' || ch == 'Q' || ch == 27) return LauncherKey::Quit;
    if (ch == 'k' || ch == 'K') return LauncherKey::Up;
    if (ch == 'j' || ch == 'J') return LauncherKey::Down;

    if (ch == 0 || ch == 224) {
        const int extended = _getch();
        if (extended == 72) return LauncherKey::Up;
        if (extended == 80) return LauncherKey::Down;
    }
...
void printRuntimeHotkeys(const DemoDefinition& definition, bool debug_mode) {
    std::cout << "Starting demo: " << definition.title() << "\n";
    std::cout << "Runtime hotkeys:\n";
    std::cout << "  d : toggle FPS overlay (ultralytics: per-tile, weapon/fire: total avg)\n";
    std::cout << "  t : toggle elapsed time banner (hidden by default)\n";
    std::cout << "  m : toggle fullscreen (starts fullscreen)\n";
    std::cout << "  left click  : enable selected tile\n";
    std::cout << "  right click : disable selected tile\n";
    std::cout << "  f : enable all tiles\n";
    std::cout << "  c : disable all tiles\n";
    std::cout << "  q / Esc : quit demo\n";
    if (debug_mode) {
        std::cout << "  --debug : per-stage inference benchmarks enabled\n";
    }
    std::cout << "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::string trace_json_path = "trace.json";
    std::string demo_id;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--debug" || arg == "-d") {
            ...
            if (i + 1 >= argc) {
                std::cerr << "Missing value for --json-path\n\n";
    ...
    setDemoVerboseLogEnabled(verbose_mode);
    printRuntimeHotkeys(*definition, debug_mode);
    if (debug_mode) runtime.setDebugMode(true);
    if (debug_mode) mobilint::startTracingEvents(trace_json_path.c_str());
    runtime.run();
    if (debug_mode) mobilint::stopTracingEvents();
    return 0;
}
