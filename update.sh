...
DEMO_NAME="ARIES CV Demo"
DEMO_DIR_NAME="aries-cv-demo"
...
echo "Downloading CV assets (mxq/mp4) from Hugging Face buckets..."
run_as_user bash ./download.sh
...
if [ -f /usr/share/applications/aries_cv_demo.desktop ]; then
  sudo rm /usr/share/applications/aries_cv_demo.desktop
fi
if [ -f /usr/share/applications/aries-cv-demo.desktop ]; then
  sudo rm /usr/share/applications/aries-cv-demo.desktop
fi
